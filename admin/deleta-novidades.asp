...
oConexaoDelete.AbreTabela("select fotoCapa,fotoTopoPagina from "&oConexaoDelete.prefixoTabela&"novidades where ativo='s' and id="&id)
nomeDoArquivo = oConexaoDelete.rs("fotoCapa")
nomeDoArquivo2 = oConexaoDelete.rs("fotoTopoPagina")

    dim fs
    Set fs=Server.CreateObject("Scripting.FileSystemObject")    
    MyFile = Server.mappath(upGalerias&"__media\novidades\"&nomeDoArquivo)
...
'apagando registro
oConexaoDelete.UpdateItem("DELETE from "&oConexaoDelete.prefixoTabela&"novidades WHERE id="&id)

response.redirect("lista-novidades.asp?msg_sys=45")
