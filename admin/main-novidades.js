...
// Foto para capa Projeto
var myDropzoneFotoCapaProjeto = new Dropzone("#my-dropzone-fotoCapaProjeto", {
    addRemoveLinks: true,
    parallelUploads: 1,
    uploadMultiple: true,
    maxFiles: 1,
...
$('#my-dropzone-fotoCapaProjeto').on('click', '.js-open-cropper-modal', function (e) {
    e.preventDefault();
    ...
    var modalTemplate =
        ...
        '<img id="img-' + ++c + '" src="__media/novidades/' + fileName + '">' +
        '</div>' +
        '</div>' +
    ...
    $cropperModal.modal('show').on("shown.bs.modal", function () {
        ...
        $this
            .on('click', '.crop-upload', function () {
                ...
                var files = myDropzoneFotoCapaProjeto.getAcceptedFiles();
                for (var i = 0; i < files.length; i++) {
                    var file = files[i];
                    if (file.name === fileName) {
                        myDropzoneFotoCapaProjeto.removeFile(file);
                    }
                }
                myDropzoneFotoCapaProjeto.addFile(croppedFile);
                $this.modal('hide');
            })
...
// Foto para capa Projeto
var myDropzoneFotoTopoProjeto = new Dropzone("#my-dropzone-fotoTopoProjeto", {
    addRemoveLinks: true,
    parallelUploads: 1,
    uploadMultiple: true,
    maxFiles: 1,
...
$('#my-dropzone-fotoTopoProjeto').on('click', '.js-open-cropper-modal', function (e) {
    e.preventDefault();
    var fileName = $(this).data('file-name');
    ...
    $cropperModal.modal('show').on("shown.bs.modal", function () {
        ...
        $this
            .on('click', '.crop-upload', function () {
                ...
                var files = myDropzoneFotoTopoProjeto.getAcceptedFiles();
                for (var i = 0; i < files.length; i++) {
                    var file = files[i];
                    if (file.name === fileName) {
                        myDropzoneFotoTopoProjeto.removeFile(file);
                    }
                }
                myDropzoneFotoTopoProjeto.addFile(croppedFile);
                $this.modal('hide');
            })
            .on('click', '.rotate-right', function () {
                cropper.rotate(90);
