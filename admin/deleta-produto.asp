...
oConexaoDelete.AbreTabela("select foto from "&oConexaoDelete.prefixoTabela&"produtos where ativo='s' and id="&id)
nomeDoArquivo = oConexaoDelete.rs("foto")

    dim fs
    Set fs=Server.CreateObject("Scripting.FileSystemObject")    
    MyFile = Server.mappath(upGalerias&"__media\produtos\"&nomeDoArquivo)
...
'apagando registro
oConexaoDelete.UpdateItem("DELETE from "&oConexaoDelete.prefixoTabela&"produtos WHERE id="&id)

response.redirect("lista-produtos.asp?msg_sys=45")
